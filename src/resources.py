...
wrong_arg_message: dict = dict(
    cod=404, message="Wrong arguments provided. For more info..."
)
no_data_message: dict = dict(
    cod=404, message="There is no existing record for the date provided"
)
...
def format_responses(response: list[tuple]) -> dict:
    ...
    for comp in response:
        if comp[1]:
            is_empty = False
            break
    if not response or is_empty:
        raise ValueError

    final_data = []
    for company_name, rows in response:
        company_data = dict(company=company_name, cod=200, data=[])
        for row in rows:
            temp_date_data = dict(
                Date=row[0],
                Open=row[1],
